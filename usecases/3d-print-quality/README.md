...
段階と根拠の区分は[検証状態](../../docs/ja/verification-status.md)にある。

- プロンプト精度（実測）: 公開ドキュメント中の実写 4 枚で 4/4、テキストで書いた症状の記述
  5 件で 5/5。測っているものが違うので合算しない
- レイテンシ（実測）: Haiku 平均 1,417 ms + Sonnet 平均 7,186 ms。n=4、逐次実行、
  クライアント側での計測。2026-05-29 / ap-northeast-1
- 2 段階分析のコスト: 削減幅は異常率で決まる。月額は
  [コストモデル](../../docs/ja/cost-model.md) の式で計算する。以前ここにあった月 $40 / $259 は
  出典の異なる単価から来ていて再現できなかったため撤回した。請求実績はない
