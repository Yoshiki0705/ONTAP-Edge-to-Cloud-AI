...
3. **FSx for ONTAP S3 AP usage pattern**: Using FSx for ONTAP as the data aggregation destination, with S3 AP providing direct connectivity to Athena, Glue and Bedrock Knowledge Bases, enables cross-analysis without data copying (SageMaker is not on the AWS list of supported services and is unverified; see [S3 AP compatibility and limits](s3ap-compatibility-matrix.md))
...
| **Data flow** | Pi → NFS → ONTAP → SnapMirror → FSx for ONTAP → S3 AP (SageMaker connectivity unverified) → SageMaker |
...
| **Data flow** | Pi (edge inference: anomaly score) → NFS → ONTAP → SnapMirror → FSx for ONTAP → S3 AP (SageMaker connectivity unverified) → SageMaker |
...
- Sensor data: Pi → NFS → ONTAP → SnapMirror → FSx for ONTAP → S3 AP → Athena / SageMaker (SageMaker connectivity unverified)
