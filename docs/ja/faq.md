...
| 対象 | 関心 |
|------|------|
| IoT/エッジ開発者 | デバイスが生成するデータの集約・活用方法 |
| データ活用推進者 | 分散データのサイロ化を解消し組織横断で分析 |
| 既存 ONTAP ユーザー | ONTAP を IoT データ集約先として活用 |
| AWS ユーザー | S3 以外のストレージから Athena/Bedrock/SageMaker を利用 |

**Q: ONTAP がない場合、別のストレージで同じことはできますか？**

A: はい。コアのパターン（エッジ収集 → 集約 → AI 分析）は他のストレージでも実現可能です。

| ストレージ | データフロー | 特徴 | 制約 |
|-----------|------------|------|------|
| **S3 直接** | エッジ → S3 → Athena/Bedrock | 最もシンプル。AWS ネイティブ統合。S3 Object Lock で改ざん防止 | NFS/SMB 非対応。既存ファイルワークフローとの統合に別途工夫が必要 |
| **EFS** | エッジ → NFS → EFS → Lambda/Bedrock | NFS マウント可能。Linux デバイスと親和性が高い。自動スケール | SMB 非対応。S3 API 直接アクセス不可。リージョン間は EFS Replication |
| **ONTAP** | エッジ → NFS/SMB → ONTAP → S3 AP → AWS AI | NFS + SMB + S3 を同一データで提供。FPolicy トリガー。SnapMirror 差分同期。ARP/AI ランサムウェア保護 | ONTAP 環境が必要。S3 AP は条件付き書き込み非対応。運用に ONTAP の知識が必要 |
