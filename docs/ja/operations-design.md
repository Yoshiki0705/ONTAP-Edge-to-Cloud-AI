...
| アップロード成功率 | アップロード試行のうち成功した割合 | `uploads_success / uploads_total` | エッジ → クラウド |
...
| アップロード成功率 | ≥ 99.0% | 30日間 | 14.4時間/月 (バッファで吸収) |
...
| アップロード成功率 < 99.0% | ネットワーク確認、SORACOM ステータス確認、バッファ状態確認 |
